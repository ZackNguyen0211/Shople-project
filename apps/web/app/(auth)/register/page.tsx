...
export default function RegisterPage({ searchParams }: { searchParams: { error?: string; next?: string } }) {
  ...
  return (
    <div className="auth-wrap">
      <form action="/api/auth/register" method="post" className="card auth-card">
        ...
        <label style={{ display: 'grid', gap: 4 }}>
          ...
          <input name="password" type="password" required minLength={8} onChange={(e) => setPw(e.target.value)} className="input" />
        </label>
        <div style={{ height: 6, background: '#e5e7eb', borderRadius: 999 }}>
          <div style={{ width: `${pct}%`, height: '100%', background: color, borderRadius: 999 }} />
        </div>
        <p className="muted" style={{ fontSize: 12 }}>{t.auth.quickSecure}</p>
        <label style={{ display: 'grid', gap: 4 }}>
          <span>{t.auth.confirmPassword}</span>
          <input name="confirm" type="password" required onChange={(e) => setConfirm(e.target.value)} className="input" />
        </label>
        {confirm ? (
        ...
        <a href={next ? `/login?next=${encodeURIComponent(next)}` : '/login'} style={{ color: 'var(--primary)' }}>
          {t.auth.loginPrompt}
        </a>
      </form>
    </div>
